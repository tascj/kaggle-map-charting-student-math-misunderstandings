...
class MAPDataset(Dataset):
    def __init__(
        ...
        df = pd.read_parquet(parquet_file).fillna("NA")
    ...
    def _getitem(self, idx, format):
        ...
        suffix_ids.append(torch.full_like(suffix, suffix_id))
        suffix_id += 1
        suffix_labels = suffix[1:]
        total_length = sum(_.numel() for _ in input_ids)
        suffix_masks = torch.zeros(total_length, dtype=torch.bool)
        suffix_masks[-suffix.numel() : -1] = True

        last_tokens = torch.tensor([_.size(0) for _ in input_ids]).cumsum(0)[1:-1] - 1
    ...
    def collate_fn(self, samples, block_size=128):
        ...
        suffix_labels = torch.cat([sample["suffix_labels"] for sample in samples])
        suffix_masks = torch.cat([sample["suffix_masks"] for sample in samples])
        ...
        suffix_masks = F.pad(suffix_masks, [0, pad_size], value=0)
...
if __name__ == "__main__":
    ...
    ds = MAPDataset(
        parquet_file="../artifacts/dtrainval_qwen3_235b_a22b_thinking_2507_fp8.parquet",
        tokenizer=tokenizer,
        query="(fold in [0, 1, 2, 3]) and (~short_response.str.contains('re-evaluation'))",
        train_mode=True,
    )
