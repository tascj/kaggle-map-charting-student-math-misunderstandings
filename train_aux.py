    ...
    for curr_epoch in range(max_epochs):
        ...
        for curr_iter, batch in enumerate(train_loader):
            ...
            for _logits, _label in zip(
                ...
                losses_cls.append(F.cross_entropy(_logits, _label))
            loss_cls = torch.stack(losses_cls).mean()
            loss = loss_cls + sft_loss
            optimizer.ready_for_optimizer_step = True
            loss.backward()
            batch_loss_cls += loss_cls.detach()
            batch_loss_sft += sft_loss.detach()
            ...
            if total_updates % cfg.train.log_interval == 0:
                ...
                loss_val_cls = batch_loss_cls.item()
                loss_val_sft = batch_loss_sft.item()
                logger.info(
                    f"Epoch [{curr_epoch + 1}/{max_epochs}] Iter [{curr_iter + 1}/{len(train_loader)}]"
                    f" lr: {lr:.4e}, loss_cls: {loss_val_cls:.4f}, loss_sft: {loss_val_sft:.4f},"
